              ...
              <div className="space-y-3">
                ...
                <div className="space-y-4 text-right">
                  <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`w-3 h-3 rounded-full ${supabaseStatus.connected ? 'bg-emerald-500' : 'bg-red-500'} ${supabaseStatus.connected ? 'animate-pulse' : ''}`}></span>
                      <span className="text-xs font-bold text-stone-700">
                        {supabaseStatus.connected ? 'متصل بنجاح مع Supabase' : 'فشل الاتصال / الجداول غير جاهزة'}
                      </span>
                    </div>
                    <button 
                      type="button"
                  ...
                  <div className="grid grid-cols-2 gap-2" dir="rtl">
                    <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl">
                      <span className="block text-[10px] text-stone-400 font-bold">المستخدمين (users)</span>
                      <span className="text-xs font-bold text-stone-800">{supabaseStatus.users} سجلات</span>
                    </div>
                    <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl">
                      <span className="block text-[10px] text-stone-400 font-bold">الأسماء (contacts)</span>
                      <span className="text-xs font-bold text-stone-800">{supabaseStatus.contacts} سجلات</span>
                    </div>
                    <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl">
                      <span className="block text-[10px] text-stone-400 font-bold">الرسائل (messages)</span>
                      <span className="text-xs font-bold text-stone-800">{supabaseStatus.messages} سجلات</span>
                    </div>
                    <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl">
                      <span className="block text-[10px] text-stone-400 font-bold">المكالمات (calls)</span>
                      <span className="text-xs font-bold text-stone-800">{supabaseStatus.calls} سجلات</span>
                    </div>
                  </div>

                  {syncMessage && (
                    <div className={`p-3 text-xs rounded-xl border ${
                      syncMessage.includes('بنجاح') 
                  ...
                  <button
                    ...
                    onClick={async () => {
                      setIsSyncing(true);
                      setSyncMessage('🔄 جاري البدء في نقل ومزامنة البيانات إلى Supabase...');
                      try {
                        // 1. Sync current user
                        if (currentUser.id) {
                          await syncUserToSupabase(currentUser as any);
                        }
                        ...
                        for (const call of callHistory) {
                          await syncCallToSupabase(call);
                        }

                        setSyncMessage('✅ تم رفع وتحديث ومزامنة كافة البيانات بالكامل إلى جداول Supabase بنجاح!');
                        await fetchSupabaseStatus();
                      } catch (err: any) {
                        setSyncMessage(`⚠️ خطأ أثناء المزامنة: ${err.message || String(err)}. يرجى التأكد من تشغيل كود SQL أولاً لتهيئة الجداول.`);
                      } finally {
                        setIsSyncing(false);
                      }
                    }}
                    disabled={isSyncing}
                    className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
                    <span>{isSyncing ? 'جاري المزامنة...' : 'مزامنة ورفع البيانات الحالية لـ Supabase'}</span>
                  </button>

                  <div className="space-y-2 border-t border-[#E5E1D8] pt-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-[#556B2F]">📜 كود تهيئة الجداول في Supabase SQL</span>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="p-1.5 bg-stone-100 hover:bg-stone-200 text-stone-600 rounded-lg transition flex items-center gap-1 text-[10px]"
                        title="نسخ كود SQL"
                      >
                    ...
                    <p className="text-[10px] text-stone-400">
                      انسخ الكود أدناه والصقه في قسم **SQL Editor** بداخل لوحة تحكم Supabase لتأسيس الجداول وتفعيل الصلاحيات (RLS) تلقائياً:
                    </p>
                    <pre className="p-2.5 bg-stone-900 text-stone-200 rounded-xl text-[10px] font-mono overflow-y-auto max-h-[120px] text-left select-all" dir="ltr">
                      {SUPABASE_SQL_SCHEMA}
                    </pre>
                  </div>
                </div>
